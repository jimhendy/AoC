...
class UltraCrucible(State):
    ...
    def _can_go_forwards(self):
        return self._num_forward_steps() < 9
    ...
    def _valid_location(location: Point2D) -> bool:
        return (0 <= location.x <= UltraCrucible.max_x) and (
            0 <= location.y <= UltraCrucible.max_y
        )
...
def run(inputs: str) -> int:
    UltraCrucible.grid = list(list(map(int, line)) for line in inputs.splitlines())
    UltraCrucible.max_x = len(UltraCrucible.grid[0]) - 1
    UltraCrucible.max_y = len(UltraCrucible.grid) - 1

    initial_state = UltraCrucible(position=Point2D(0, 0))
