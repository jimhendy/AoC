...
def enhance_sub_image(sub_image, rules):
    for rule in rules:
        ...
        if any(
            [
                np.array_equal(sub_image, m)
                for m in find_possible_matches(rule_i)
            ]
        ):
            return rule[1]
...
def enhance(image, rules):
    ...
    for y_start in range(0, size, break_size):
        ...
        for x_start in range(0, size, break_size):
            sub_image = image[y_start:y_start+break_size, x_start:x_start+break_size]
            this_row.append(
                enhance_sub_image(sub_image, rules)
            )
