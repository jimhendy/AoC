...
class Assembunny:
    ...
    def __call__(self):
        while len(self.transmission_output) < 50:
            ...
            if (
                len(self.transmission_output) > 1
                and
                (
                    any([i!=0 for i in self.transmission_output[::2]])
                    or
                    any([i!=1 for i in self.transmission_output[1::2]])
                )
            ):
            ...
            if func_name == 'tgl':
                ...
                if func_name == 'jnz':
                   new_func = 'cpy'
                    ...
                    new_func = 'jnz'
            logger.debug(f'Toggling position {tgl_pointer}, {func_name} to {new_func} with args {args}')
            self.instructions[tgl_pointer] = [new_func] + args

        self.instruction_pointer += 1

    def out(self, x):
        self.transmission_output.append(
            self.get_value(x)
        )
        self.instruction_pointer += 1
    ...
    def check_for_cheat(self):
        ...
        logger.debug(f'Implementing cheat, {a} += {b} * {d} ({a_value} += {b_value} * {d_value})')
