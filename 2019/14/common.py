...
def find_reaction(reactions, element):
    ...
    if not len(possibles) == 1:
        if len(possibles) == 0:
            ...
            ex = Exception
        raise ex(
            f'Found {len(possibles)} reactions producing {element} : {possibles}'
        )
    return possibles[0]
        ...
        for ic in reaction.inputs:
            if not store.is_available(ic):
                ...
                num_reactions_required = int(
                    np.ceil(
                        num_required_from_reactions / input_reaction.output.num
                    )
                )
